...
Rails.application.config.middleware.use OmniAuth::Builder do
  ...
  provider :facebook, "#{Settings.facebook.APP_ID}", "#{Settings.facebook.APP_SECRET_ID}", :scope => 'email,read_stream', :display => 'popup'
  provider :google_oauth2, "#{Settings.gmail.APP_ID}", "#{Settings.gmail.APP_SECRET_ID}", scope: 'userinfo.email, userinfo.profile'
end
class VideoAnalytic < ActiveRecord::Base
	belongs_to :move
	belongs_to :user

	def self.update_video_info(video_id, move, user)
		if video_id.present?
			video_info = VideoAnalytic.find(video_id)
			completed_video = VideoAnalytic.where(move_id: move.id, status: "completed")
			...
			if last_video_info.present?
				...
				video_info = VideoAnalytic.new(move_id: move.id, user_id: user.id, view_start_time: DateTime.now, status: "playing")
				video_info.save
		end
		return video_info
	end
